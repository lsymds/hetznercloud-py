from hetznercloud import SERVER_STATUS_RUNNING, VOLUME_STATUS_AVAILABLE, VOLUME_MINIMUM_SIZE, ACTION_STATUS_SUCCESS
from tests.base import BaseHetznerTest


class TestVolumes(BaseHetznerTest):
    def test_can_create_a_volume_in_location(self):
        volume, _ = self.create_volume(name = "test-can-create-a-volume-in-location", location = "nbg1")

        self.assertIsNotNone(volume)
        self.assertIsNotNone(volume.id)
        self.assertEqual(volume.size, VOLUME_MINIMUM_SIZE)
    ...
    def test_can_create_a_volume_with_server(self):
        ...
        volume, _ = self.create_volume(name = "test-can-create-a-volume-with-server", server_id = server.id)
    ...
    def test_can_resize_a_volume(self):
        volume, action = self.create_volume(name = "test-can-resize-a-volume", location = "nbg1")
        action.wait_until_status_is(ACTION_STATUS_SUCCESS)

        action = volume.resize(VOLUME_MINIMUM_SIZE * 2)
        action.wait_until_status_is(ACTION_STATUS_SUCCESS)
    ...
    def test_can_attach_and_detach_a_volume_to_a_server(self):
        ...
        volume, _ = self.create_volume(name = "test-can-attach-and-detach-a-volume-to-a-server", location = "nbg1")
        volume.wait_until_status_is(VOLUME_STATUS_AVAILABLE)
